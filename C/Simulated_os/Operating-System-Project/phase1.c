...
Process *dequeue();
void  enqueue(int id);
void switch_context(int old_pid, int new_pid,  int status);
int init_main(void *);
int is_kernel_mode();
void blockMe();
...
//void TEMP_switchTo(int pid) {
 // unsigned int oldPSR = disable_interrupts();
  ...
  //printf("psr(after): %d\n",USLOSS_PsrGet());
/*  if (pid < 0 || !PCB[pid % MAXPROC].allocated) {
    USLOSS_Console("Error: Invalid process switch to pid %d\n", pid);
    USLOSS_Halt(1);
  }
  Process * oldProc = currProcRunning;
  Process * newProc = &PCB[pid % MAXPROC];
  currProcRunning = newProc;
  if (!oldProc){
	USLOSS_ContextSwitch(NULL, &PCB[pid % MAXPROC].context);
  }
  //currProcRunning = &PCB[pid];
  else{
	USLOSS_ContextSwitch(&oldProc->context, &newProc->context);
  }
  //currProcRunning = &PCB[pid % MAXPROC];
  /*if (currProcRunning){ 
	printf("old proc %s to new proc %s\n", oldProc->name, currProcRunning->name);
  }
  else {
	  printf("curr proc was NULL\n");
  }*/
  //printf("currProcRunning(225): %s\n",currProcRunning->name);
  //Something something set PSR oldPSR?
  //oldPSR++; // this negates the warning for unused var....
//}
...
void dispatcher(){
  ...
  *queue_head = next->next_priority;
  ...
  if(currProcRunning != NULL && currProcRunning->process_state == RUNNING)
	add_To_Priority_Queue(currProcRunning);
	
	
  // Timer Stuff?  
  printf("\tdispatcher is switching from %d -> %d\n",((currProcRunning)?currProcRunning->pid:-1), next->pid );
