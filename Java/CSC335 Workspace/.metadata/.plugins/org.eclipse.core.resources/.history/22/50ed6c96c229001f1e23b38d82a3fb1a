...
public class ArrListTest {
	...
	System.out.println("\nExpected Score: " + Math.min(80, score));
    }
	...
	if(checkCounts("addLast", 3*numToAdd + 2*exp.size()) && checkLists()) {
	    score += 8;
	}
	return score;
    }
	...
	if(checkCounts("addFirst", 3*numToAdd + 2*exp.size()) && checkLists()) {
	    score += 8;
	}
	return score;
    }
	...
	for(int i = 0; i < 2; i++) {
		...
		exp.add(j, 999+i);
		act.add(j, 999+i);
		if(checkCounts("add", 3*exp.size()) && checkLists())
		    score += 2.0;
	    } catch (Exception e) {
		e.printStackTrace();
	    }
	...
	if(passedAccuracy && passedCounts)
	    score += 8;
	return score;
    }
	...
	if(checkCounts("removeFirst", 50) && checkLists()) {
	    score += 8;
	}
	return score;
    }
	...
	if(checkCounts("removeLast", 50) && checkLists()) {
	    score += 8;
	}
	return score;
    }
	...
	if(checkCounts("removeByIndex", 75*exp.size()) && checkLists()) {
	    score += 4;
	}
	return score;
    }
	...
	if(checkCounts("removeByValue", 50*exp.size()) && checkLists()) {
	    score += 4;
	}
	...
	if(checkCounts("removeRange", 2*(s + j-i)) && checkLists())
	    score += 8;
	return score;
    }
	...
	if(checkCounts("set", 20) && checkLists()) {
	    score += 2.0;
	}
	return score;
    }
